...
public class GGEveDBAdapter {
	...
	public static final int COLUMN_ACCOUNT_APIKEY = 2;

	private SQLiteDatabase myDb;
	private final Context myContext;
	private DbHelper myDbHelper;

	private static final String DATABASE_CREATE_CHARACTER_TABLE = "create table " + DATABASE_CHARACTER_TABLE + " (" + KEY_CHAR_TABLE_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
	...
	private static final String DATABASE_CREATE_ACCOUNTS_TABLE = "create table " + DATABASE_ACCOUNTS_TABLE + " (" + KEY_ACCOUNT_TABLE_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "
			+ AccountDetails.KEY_ACCOUNT_USERID + " TEXT NOT NULL, "
			+ AccountDetails.KEY_ACCOUNT_APIKEY + " TEXT NOT NULL);";
	
	private static final String DATABASE_CREATE_TRAINING_INFO_TABLE = "create table " + DATABASE_TRAINING_INFO_TABLE + " (" + KEY_TRAINING_TABLE_ID + " INTEGER PRIMARY KEY AUTOINCREMENT,"
			+ TrainingInformation.KEY_CACHED_UNTIL + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_CHARACTER_ID + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_CURRENT_TIME + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_SKILL_ID + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_SKILL_IN_TRAINING + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_TRAIN_END_SP + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_TRAIN_END_TIME + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_TRAIN_START_SP + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_TRAIN_START_TIME + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_TRAIN_TO_LEVEL + " TEXT NOT NULL,"
			+ TrainingInformation.KEY_TRAIN_TYPE_ID + " TEXT NOT NULL);";
	...
	public Vector<AccountDetails> getAccounts()
	{
		...
		while (keepGoing) {
			...
			String apikey = cursor.getString(COLUMN_ACCOUNT_APIKEY);
			returnValue.add(new AccountDetails(apikey, userid));
	...
	public AccountDetails getAccount(int inUserID)
	{
		...
		int userid = cursor.getInt(COLUMN_ACCOUNT_USERID);
		String apikey = cursor.getString(COLUMN_ACCOUNT_APIKEY);
		return new AccountDetails(apikey, userid);
	}
	...
	public long updateAccountDetails(AccountDetails inAccountDetails)
	{
		...
		contentValues.put(AccountDetails.KEY_ACCOUNT_APIKEY, inAccountDetails.getAPIKey());
		if (accountExists)
		{
	...
	public Vector<TrainingInformation> getTrainingInformation()
	{
		//for each account
		//add the training information for current train to the vector and return it.
		return null;
	}
	
	public void updateTrainingInformation(TrainingInformation inTrainingInformation)
	{
	...
	private long addNewEveCharacter(EveCharacter inEveCharacter) {
		...
		return myDb.insert(DATABASE_CHARACTER_TABLE, null, contentValues);
	}
	...
	private int updateNewEveCharacter(EveCharacter inEveCharacter) {
		...
		if (inEveCharacter.getBalance() != -1)
			contentValues.put(EveCharacter.KEY_CHARACTER_BALANCE, inEveCharacter.getBalance());
		return myDb.update(DATABASE_CHARACTER_TABLE, contentValues, where, null);
	}
